...
to_rover_direction = (0,0)

bridge = CvBridge()

FOLLOWING_SPEED_KOEF = 1/160
FOLLOWING_TOLERANCE = 20
...
def image_callback(msg):
    ...
    if len(results) > 0:
        ...
        if rover_pixel_x is not None and rover_pixel_y is not None:
            x_diff = rover_pixel_x - img_center[0]
            y_diff = rover_pixel_y - img_center[1]

            print("diffs")
            print(x_diff, y_diff)

            if (x_diff < FOLLOWING_TOLERANCE) and (y_diff < FOLLOWING_TOLERANCE):
                x_diff = 0
                y_diff = 0
            
            to_rover_direction = (x_diff*FOLLOWING_SPEED_KOEF, y_diff*FOLLOWING_SPEED_KOEF)
...
def main():
    ...
    while not rospy.is_shutdown():
        set_velocity(vx=to_rover_direction[0], vy=to_rover_direction[1], vz = 0, frame_id='body')
